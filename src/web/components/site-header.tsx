...
export function SiteHeader({ gameId, busy, onCopy, onRefresh, onExit }: SiteHeaderProps) {
  return (
    <header className="flex min-h-18 items-center justify-between gap-4 border-b border-white/10 bg-emerald-950/90 px-4 py-3 sm:px-8">
      ...
      {gameId !== null && (
        <div className="flex min-w-0 items-center gap-1 sm:gap-2">
          ...
          <Button aria-label="Exit game" className="w-11 px-0 sm:w-auto sm:px-3" onClick={onExit} type="button" variant="danger"><LogOut aria-hidden="true" size={18} /><span className="hidden sm:ml-2 sm:inline">Exit</span></Button>
        </div>
      )}
    </header>
  );
